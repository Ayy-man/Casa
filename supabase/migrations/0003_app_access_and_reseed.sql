-- ============================================================================
-- Casa Command Center — App Access + Idempotent Re-seed (Phase 2, recovery)
-- Project: aqsitrzbjokkkpcohple
-- ----------------------------------------------------------------------------
-- WHY THIS FILE EXISTS:
-- 0001 (schema) and 0002 (seed) were applied, but the 12 tables are not
-- readable by the app's anon / publishable key — REST count queries return 0
-- rows for every table even though `properties` clearly has rows (a re-run of
-- 0002 failed with `duplicate key ... (id)=(p01) already exists`). Cause:
-- Row-Level Security with no policy, and/or missing role grants.
--
-- PROJECT.md explicitly defers RLS + real auth to v2 — this milestone runs
-- WITHOUT RLS (single-tenant, shared demo creds). So the 12 tables must be
-- readable now (and writable, for the Phase 3 action lifecycle) by the `anon`
-- and `authenticated` roles.
--
-- This script is idempotent and safe to re-run:
--   Part A — disable RLS on all 12 tables + grant CRUD to anon/authenticated
--   Part B — TRUNCATE all 12 tables (clean slate, regardless of prior partial seed)
--   Part C — re-seed the 6 seeded tables (verbatim from 0002_seed.sql)
--
-- Run this file ALONE. Do NOT also run 0002_seed.sql — Part C below replaces it.
-- ============================================================================

-- ── Part A: app access (no-RLS milestone) ──────────────────────────────────
...
-- Apply the same grants to any future tables created in this schema.
...
-- ── Part C: re-seed (verbatim from 0002_seed.sql) ───────────────────────────
-- ----------------------------------------------------------------------------
